...
Yes. There is a 
Job Satisfaction is mostly determined by HourlyRate and the Number of Companies Worked
...
#Identify Predictive Variables for Life Satisfaction
model <- lm(JobSatisfaction ~ ., data = MyPredictionData)
JobSatisfaction.lm<-ols_step_both_p(model)
#HourlyRate and NumCOmpanies Worked are best predictors
fitmodel <- lm(JobSatisfaction ~ HourlyRate+NumCompaniesWorked , data = MyPredictionData)
JobSatisfaction.lm<-ols_step_both_p(model)

plot(JobSatisfaction.lm)
plot(fitmodel)
...
#Other Methods of Variable Selection that were Attempted
